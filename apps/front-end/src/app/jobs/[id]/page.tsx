          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              <div className="rounded-xl border border-white/10 bg-slate-900/40 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">发布人</p>
                <TooltipProvider>
                  <Tooltip>
              ...
              <div className="rounded-xl border border-white/10 bg-slate-900/40 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">分类</p>
                <p className="mt-1 text-white font-semibold">{job.category ?? "未分类"}</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-slate-900/40 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">执行方</p>
                <p className="mt-1 text-purple-300 font-semibold">
                  {job.selectedAgentId ?? "尚未选择"}
                </p>
              </div>
              <div className="rounded-xl border border-white/10 bg-slate-900/40 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">预算</p>
      ...
      <Card className="border-white/10 bg-gradient-to-r from-slate-950/70 via-slate-950/60 to-slate-950/70">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div className="flex items-center justify-between md:justify-start md:gap-4">
            ...
            <span className="text-white font-semibold">{job.visibility}</span>
          </div>
          <div className="flex items-center justify-between md:justify-start md:gap-4">
          ...
          <div className="flex items-center justify-between md:justify-start md:gap-4">
            ...
            <span className="text-white font-semibold">{payoutStrategyLabel}</span>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="flex items-center gap-3">
                <div className="size-8 rounded-lg bg-cyan-500/10 border border-cyan-400/20 flex items-center justify-center">
                  <svg className="w-4 h-4 text-cyan-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m-6-8h6M5 5h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z" />
                  </svg>
                </div>
            ...
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center gap-3">
                <div className="size-8 rounded-lg bg-purple-500/10 border border-purple-400/20 flex items-center justify-center">
                  <svg className="w-4 h-4 text-purple-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 7l7-4 7 4v10l-7 4-7-4V7z" />
                  </svg>
                </div>
              ...
              {job.tags.length > 0 ? (
                <div className="flex gap-2 flex-wrap">
                  {job.tags.map((tag) => (
                    <Badge key={tag} variant="outline" className="border-cyan-500/20 text-cyan-100">
                      {tag}
                    </Badge>
                  ))}
                </div>
          ...
          <Card className="bg-slate-900/50 border-blue-500/20 shadow-[0_14px_40px_rgba(59,130,246,0.15)]">
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-semibold">{createdAtLabel}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">截止日期</span>
                <span className="text-white font-semibold">{deadlineLabel}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">支付方式</span>
                <span className="text-white font-semibold">{paymentMethodLabel}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">币种</span>
                <span className="text-white font-semibold">{job.currency ?? "—"}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">优先级</span>
                <span className="text-white font-semibold">{job.priority}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-600/15 via-slate-900/50 to-blue-600/10 border-purple-500/20">
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-semibold">{job.requiredSkillLevel}</span>
              </div>
