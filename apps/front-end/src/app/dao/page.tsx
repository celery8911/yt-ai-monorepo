					...
					(item: DisputeListItem | null, index: number) => {
						const title = isLoading
							...
							: item?.agentName && item?.jobTitle
								...
								: item?.agentName
									? item.agentName
									: (item?.jobTitle ?? `争议 #${item?.id ?? "未知"}`);
						const status = isLoading ? "OPEN" : (item?.status ?? "OPEN");
						const amount = isLoading ? undefined : item?.escrowAmount;
						const currency = isLoading ? undefined : item?.currency;
						const votesFor = isLoading ? 0 : (item?.votesFor ?? 0);
						const votesAgainst = isLoading ? 0 : (item?.votesAgainst ?? 0);
