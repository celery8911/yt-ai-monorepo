...
const Billing = () => {
	...
	const { totalIncome, totalSpent, monthlyIncome, availableMonths } =
		useMemo(() => {
			if (!bills)
				return {
					totalIncome: 0,
			...
			const income = bills
				.filter(
					(bill) =>
						bill.payeeAddress === CURRENT_USER_ADDRESS &&
						bill.status === "PAID",
				)
			...
			const spent = bills
				.filter(
					(bill) =>
						bill.payerAddress === CURRENT_USER_ADDRESS &&
						bill.status === "PAID",
				)
				.reduce((sum, bill) => sum + bill.amount, 0);
