...
export const formatJobBudget = (job: Pick<Job, "paymentMethod" | "budgetMin" | "budgetMax" | "currency">): string => {
...
export const fetchJobList = async (filters: JobListFilters = {}): Promise<{
  items: JobListItem[];
  page: number;
  limit: number;
  total: number;
...
export const selectJobAgent = async (
  jobId: string,
  agentId?: string
): Promise<{ job: Job; selectedAgentId?: string; matches: Array<{ id: string }> }> => {
  return request({
