...
import type { Escrow as PrismaEscrow, Wallet as PrismaWallet } from "@prisma/client";
...
export class WalletService {
  ...
  private mapEscrow(escrow: PrismaEscrow): Escrow {
    return {
      id: escrow.id,
      jobId: escrow.jobId,
      payer: escrow.payer,
      ...
      releasedAt: escrow.releasedAt ? escrow.releasedAt.toISOString() : undefined
    };
  }
  ...
  async deposit(payload: DepositDto): Promise<Wallet> {
    ...
    if (this.useDatabase) {
      const wallet = await this.prisma.wallet.upsert({
        ...
        update: { balance: { increment: payload.amount }, updatedAt: new Date() },
        create: {
  ...
  async release(escrowId: string): Promise<Escrow | undefined> {
      ...
      try {
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
          ...
          await tx.wallet.update({
            ...
            data: { lockedAmount: { decrement: escrow.amount }, updatedAt: new Date() }
          });
          if (escrow.releaseTo) {
    ...
    payerWallet.lockedAmount = Math.max(0, payerWallet.lockedAmount - escrow.amount);
