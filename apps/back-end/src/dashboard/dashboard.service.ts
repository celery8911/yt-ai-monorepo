...
export class DashboardService {
	...
	async overview(agentAddress?: string, userAddress?: string) {
		...
		const agentWallet = agentAddress
			? await this.walletService.balance(agentAddress)
			: undefined;
		const userWallet = userAddress
			? await this.walletService.balance(userAddress)
			: undefined;
		...
		return {
			...
			mySuccessRate:
				agents.find((agent) => agent.owner === agentAddress)?.successRate ?? 0,
			myActiveJobs: jobs.filter((job) => job.selectedAgentId === agentAddress)
				.length,
			myTotalSpent: userWallet?.totalSpent ?? 0,
			myPublishedJobs: jobs.filter((job) => job.createdBy === userAddress)
				.length,
			myCompletedJobs: jobs.filter(
				(job) => job.createdBy === userAddress && job.status === "COMPLETED",
			).length,
		};
	}
	...
	async getStats(address: string) {
		...
		let signedAgentsCount = 0;
		try {
			const { engagements } =
				await this.chainStatusService.getEngagementsByUser(address);
			const activeEngagements = engagements.filter(
				(engagement) => engagement.status !== "CANCELLED",
			);
			signedAgentsCount = new Set(
				activeEngagements.map((engagement) => engagement.agentId),
			).size;
		} catch {
			signedAgentsCount = 0;
		}
		if (!this.useDatabase) {
			...
			const publishedAgentsCount = agents.filter(
				(agent) => agent.owner === address,
			).length;
			return {
				...
				activeJobsCount: publishedJobs.filter((job) =>
					activeStatuses.includes(job.status),
				).length,
				completedJobsCount: publishedJobs.filter(
					(job) => job.status === "COMPLETED",
				).length,
		...
		const publishedJobsCount = await this.prisma.job.count({
		...
		const publishedAgentsCount = await this.prisma.agent.count({
			where: { owner: address },
		});
	...
	async getPublishedJobs(address: string, page = 1, limit = 10) {
		if (!this.useDatabase) {
			...
			return {
				data,
				pagination: this.buildPagination(page, limit, filtered.length),
			};
		}
		...
		const selectedAgentIds = jobs
			.map((job) => job.selectedAgentId)
		...
		const bidCountByJob = new Map(
			bidCounts.map((bid) => [bid.jobId, bid._count._all]),
		);
		const data = jobs.map((job) => ({
			...
			selectedAgentName: job.selectedAgentId
				? agentNameById.get(job.selectedAgentId)
				: undefined,
		...
		if (!this.useDatabase) {
			...
			const data = paged.map((agent) => {
				const agentJobs = jobs.filter(
					(job) => job.selectedAgentId === agent.id,
				);
				const activeJobsCount = agentJobs.filter((job) =>
					["OPEN", "MATCHING", "IN_PROGRESS"].includes(job.status),
				).length;
				const completedJobsCount = agentJobs.filter(
					(job) => job.status === "COMPLETED",
				).length;
				return {
					id: agent.id,
			...
			return {
				data,
				pagination: this.buildPagination(page, limit, ownedAgents.length),
			};
		}
		...
		for (const group of jobGroups) {
			...
			if (group.status === "COMPLETED") {
				...
				activeJobsByAgent.set(
					...
					(activeJobsByAgent.get(group.selectedAgentId) ?? 0) +
						group._count._all,
				);
			}
		}
	...
	async getSignedAgents(address: string, page = 1, limit = 10) {
		const fetchSize = Math.max(50, page * limit * 2);
		const { engagements } = await this.chainStatusService.getEngagementsByUser(
			address,
			{
				first: fetchSize,
				skip: 0,
		...
		const start = (page - 1) * limit;
		const paged = activeEngagements.slice(start, start + limit);
		if (paged.length === 0) {
			return {
				data: [],
				pagination: this.buildPagination(page, limit, activeEngagements.length),
			};
		}

		const agentIds = paged.map((engagement) => engagement.agentId);
		const agents = await this.agentsService.findByIds(agentIds);
		const agentById = new Map(agents.map((agent) => [agent.id, agent]));
		const data = await Promise.all(
			paged.map(async (engagement) => {
				const agent = agentById.get(engagement.agentId);
				const jobId =
					engagement.jobId && engagement.jobId !== ""
						? engagement.jobId
						: `engagement-${engagement.id}`;
				const job = engagement.jobId
					? await this.jobsService.findById(engagement.jobId)
					: undefined;
				const contractAmount = Number(
					ethers.formatUnits(BigInt(engagement.totalPaid), 18),
				);
				return {
					jobId,
					jobTitle: job?.title ?? agent?.name ?? "直接雇佣",
					jobStatus: job?.status ?? "HIRED",
					agentId: engagement.agentId,
					agentName: agent?.name,
					author: agent?.owner ?? engagement.agentOwner,
					owner: agent?.owner ?? engagement.agentOwner,
					agentCategory: agent?.category,
					agentRating: agent?.rating,
					contractAmount,
					currency: "CBT",
					contractStatus: engagement.status,
					signedAt: new Date(Number(engagement.startTime) * 1000).toISOString(),
				};
			}),
		);
		return {
	...
	async getDisputes(address: string, page = 1, limit = 10) {
		...
		const orFilters: Array<{ initiator?: string; jobId?: { in: string[] } }> = [
			{ initiator: address },
		];
		if (jobIds.length > 0) {
